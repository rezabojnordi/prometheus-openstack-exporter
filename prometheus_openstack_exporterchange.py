    ...
    if ks_version == 2:
       ...
       ## keystone = client.Client(session=sess_domain, interface=interface)
        auth = v3.Password(user_domain_name="Default",
                    username="admin",
                    password='c52b6ec9f13883f029b',
                    project_domain_name="Default",
                    project_name="admin",
                    auth_url="http://172.16.104.7:5000/v3")
        sess = session.Session(auth=auth)
        keystone = client.Client(session=sess, interface=interface)
...
class Cinder():
    def __init__(self):
        ...
        with open(config['cache_file'], 'rb+') as f:
            print("cinder read file",f.readline())
            self.prodstack = pickle.load(f)[0]
        self.tenant_map = {t['id']: t['name'] for t in self.prodstack['tenants']}
        self.use_nova_volumes = config.get('use_nova_volumes', True)
